  ...
  <body>
    ...
    <section class="card">
      <form id="plan-form">
        ...
        <select id="from-stop" required></select>
        <label for="to-stop">Arrêt d'arrivée</label>
        <select id="to-stop" required></select>
    ...
    <script>
      ...
      document.getElementById("plan-form").addEventListener("submit", async (event) => {
        ...
        const response = await fetch(`/api/plan?from_stop=${fromStop}&to_stop=${toStop}`);
...
def build_trip_options(gtfs_data, from_stop_id, to_stop_id, now_datetime, delay_map, limit=5):
    stop_times = gtfs_data["stop_times"]
    trips = gtfs_data["trips"]
    routes = gtfs_data["routes"]

    from_rows = stop_times[stop_times["stop_id"] == from_stop_id]
    to_rows = stop_times[stop_times["stop_id"] == to_stop_id]
...
def stops():
    ...
    stops_df = gtfs_data["stops"].sort_values("stop_name")
    data = stops_df[["stop_id", "stop_name"]].to_dict(orient="records")
    return jsonify(data)
...
def plan():
    ...
    trips = build_trip_options(gtfs_data, from_stop, to_stop, now_datetime, delay_map)
